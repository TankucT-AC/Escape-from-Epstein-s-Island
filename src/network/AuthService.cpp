...
AuthService::AuthService(AuthClient &c) : client(c) {
  ...
  window.create(sf::VideoMode(450, 600), "Escape From Epstein Island",
                sf::Style::Titlebar | sf::Style::Close, settings);
  window.setFramerateLimit(60);

  if (!font.loadFromFile(findSystemFont()))
    std::cerr << "Font error\n";
  setupUI();
}
...
void AuthService::setupUI() {
  ...
  secondaryBtn.init("REGISTER", {55, 430}, {340, 45}, font, ui::ColorPanelBG,
                    sf::Color(64, 68, 75));
...
void AuthService::switchState(State s) {
  ...
  secondaryBtn.isVisible = (s == State::Login || s == State::Register ||
                            s == State::Ask2FA || s == State::Verify2FA);
  ...
  if (s == State::Login) {
    ...
    infoText.setString("SAVE THIS PASSWORD! You will not see it "
    ...
    float infoBottom =
        infoText.getGlobalBounds().top + infoText.getGlobalBounds().height;
    generatedPassText.setPosition(
        225 - generatedPassText.getGlobalBounds().width / 2.f,
        infoBottom + 20.f);
    ...
    float qrBottom =
        qrSprite.getGlobalBounds().top + qrSprite.getGlobalBounds().height;
    float code2FAY = qrBottom + 10.f;
    code2FAInput.box.setPosition(125.f, code2FAY);
    // Текст внутри поля: отступ 10px от левого края и 8px от верхнего края
    // рамки
    code2FAInput.display.setPosition(code2FAInput.box.getPosition().x + 10.f,
                                     code2FAInput.box.getPosition().y + 8.f);

    float primaryBtnY =
        code2FAInput.box.getPosition().y + code2FAInput.box.getSize().y + 15.f;
...
void AuthService::handleEvents() {
  ...
  while (window.pollEvent(e)) {
    ...
    if (e.type == sf::Event::MouseButtonPressed &&
      ...
      loginInput.updateVisuals();
      passInput.updateVisuals();
      code2FAInput.updateVisuals();
      ...
      if (primaryBtn.contains(m)) {
        if (state == State::Login) {
          ...
          if (res.success) {
            ...
            setStatus("Invalid credentials!", ui::ColorError);
          ...
          if (res.success) {
            ...
            setStatus(res.message, ui::ColorError);
        } else if (state == State::Ask2FA) {
          auto res = client.setup2FA(currentLogin);
          if (res.success && !res.qr_image_data.empty()) {
            qrTexture.loadFromMemory(res.qr_image_data.data(),
                                     res.qr_image_data.size());
            ...
            float infoBottom = infoText.getGlobalBounds().top +
                               infoText.getGlobalBounds().height;
            qrSprite.setPosition(225 - qrSprite.getGlobalBounds().width / 2.f,
                                 infoBottom + 20.f);
          ...
          if (cleanDigits.empty()) {
            ...
            if (res.success)
              ...
              setStatus("Invalid 2FA code!", ui::ColorError);
          }
        }
      }
      ...
      if (secondaryBtn.contains(m)) {
        if (state == State::Login)
          ...
          authorized = true;
        else if (state == State::Verify2FA)
          switchState(State::Login); // Возврат к логину из 2FA
      }
    }
  }
}
...
void AuthService::render() {
  ...
  sf::RectangleShape p({400, 500});
  p.setPosition(25, 40);
  p.setFillColor(ui::ColorPanelBG);
  window.draw(p);
  ...
  if (state == State::Ask2FA)
    window.draw(generatedPassText);
  if (state == State::SetupQR)
