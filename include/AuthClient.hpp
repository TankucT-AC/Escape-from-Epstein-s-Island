...
public:
    ...
    virtual httplib::Result Post(const std::string& path, const std::string& body, const std::string& content_type) = 0;
};
...
public:
    ...
    httplib::Result Post(const std::string& path, const std::string& body, const std::string& content_type) override {
...
public: 
    ...
    AuthClient() : client_(std::make_unique<RealHttpClient>(config::SCHEME_HOST_PORT)) {} 
    
    // Конструктор для проброса мока в тестах
    explicit AuthClient(std::unique_ptr<IHttpClient> client) : client_(std::move(client)) {} 
