...
std::optional<DatabaseManager::UniqueStmt>
DatabaseManager::prepare(const std::string &sql) {
  sqlite3_stmt *raw_stmt = nullptr;
  if (sqlite3_prepare_v2(db.get(), sql.c_str(), -1, &raw_stmt, nullptr) !=
      SQLITE_OK) {
    return std::nullopt;
  }
...
bool DatabaseManager::insertNewUser(const std::string &login,
...
DatabaseManager::findHashPass(const std::string &login) {
  std::lock_guard<std::mutex> lock(mut);
...
std::optional<std::string>
DatabaseManager::connect2FA(const std::string &login) {
  std::lock_guard<std::mutex> lock(mut);
...
std::optional<std::string>
DatabaseManager::getTOTPSecret(const std::string &login) {
  std::lock_guard<std::mutex> lock(mut);
