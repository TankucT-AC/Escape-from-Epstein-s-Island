...
std::string TOTPGenerator::generate(const std::string &key, std::string time,
                                    const std::string &returnDigits,
                                    const std::string &crypto) {
  return TOTPGenerator::generate(TOTPGenerator::base32Decode(key), time,
                                 returnDigits, crypto);
}
...
std::string TOTPGenerator::generate(const std::vector<unsigned char> &key_bytes,
  ...
  HMAC(md, k.data(), k.size(), msg.data(), msg.size(), hash.data(), &hashLen);
  hash.resize(hashLen);
...
std::string
TOTPGenerator::base32Encode(const std::vector<unsigned char> &data) {
...
TOTPGenerator::base32Decode(const std::string &data) {
  ...
  if (padding > 0) {
    if (total_len % 8 != 0) {
      throw std::invalid_argument(
          "Invalid base32: total length not multiple of 8");
    }
  ...
  for (size_t i = 0; i < end; ++i) {
    ...
    if (bits_left >= 8) {
      bits_left -= 8;
      result.push_back(
          static_cast<unsigned char>((buffer >> bits_left) & 0xFF));
    }
  }
...
std::string TOTPGenerator::passHash(const std::string &password,
                                    const std::string &crypto) {
  ...
  if (PKCS5_PBKDF2_HMAC(
          password.c_str(), password.size(), salt.data(), salt.size(),
          600000,                                  // итерации
