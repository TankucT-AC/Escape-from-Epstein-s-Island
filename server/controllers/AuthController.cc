...
std::string generateQrCode(const std::string &uri) {
  qrcodegen::QrCode qr =
      qrcodegen::QrCode::encodeText(uri.c_str(), qrcodegen::QrCode::Ecc::LOW);
...
bool verifyPassword(const std::string &password,
  ...
  PKCS5_PBKDF2_HMAC(password.c_str(), password.size(), salt.data(), salt.size(),
                    600000, EVP_sha256(), KEYLEN, newHash.data());

  std::stringstream ss;
  for (unsigned char b : newHash)
...
void AuthController::loginUser(
  ...
  bool success = false;
  std::optional<std::string> db_hash =
      DatabaseManager::instance().findHashPass(login);
  ...
  std::optional<std::string> TOTP_secret =
      DatabaseManager::instance().getTOTPSecret(login);
  res["is_totp_verify"] = TOTP_secret.has_value();

  callback(HttpResponse::newHttpJsonResponse(res));
}

void AuthController::register2FA(
    const HttpRequestPtr &req,
    std::function<void(const HttpResponsePtr &)> &&callback) {
  std::shared_ptr<Json::Value> json = req->getJsonObject();
  Json::Value res;
  ...
  std::optional<std::string> TOTP_secret =
      DatabaseManager::instance().connect2FA(login);
  ...
  if (!TOTP_secret.has_value()) {
    ...
    std::string uri = "otpauth://totp/" + app_name + ":" + login +
                      "?secret=" + TOTP_secret.value() + "&issuer=" + app_name;
...
void AuthController::login2FA(
  ...
  if (TOTP_secret.has_value()) {
    ...
    for (int offset = -30; offset <= 30; offset += 30) {
      ...
      std::string serverTOTPkey = TOTP_gen.generate(
          TOTP_secret.value(), currHexUnixTime, "6", "HmacSHA1");

      if (serverTOTPkey.length() == TOTP_key.length() &&
          CRYPTO_memcmp(serverTOTPkey.c_str(), TOTP_key.c_str(),
                        serverTOTPkey.length()) == 0) {
        is_success = true;
      }
    }
